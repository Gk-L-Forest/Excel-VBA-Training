...
' ## 設定ファイルの読み込み
'------------------------------------------------------------------------------
Public Sub LoadConfig(ByVal config_filename As String, _
                      ByRef config_data As String)
    ...
    Open configFilePath For Input As #1
        Do Until EOF(1)
            ...
            config_data = config_data & bufferData & vbCrLf
        Loop
        config_data = Left(config_data, Len(config_data) - Len(vbCrLf))
    Close #1
